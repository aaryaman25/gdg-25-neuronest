...
const defaultHabits = [
  ...
  "🧘‍♀️ Engage in mindfulness or meditation",
...
const anxietyAdvice = [
  ...
  "Take slow, deep breaths to relax when feeling anxious.",
  "Engage in physical activity, even a short walk.",
  "Focus on what you can control in the moment.",
  "Limit caffeine or sugar intake, which can increase anxiety.",
];
...
export default function AnxietyPage() {
  ...
  useEffect(() => {
    const savedLog = localStorage.getItem("anxietyHabitLog");
    const savedCustom = localStorage.getItem("anxietyCustomHabits");
    const savedMood = localStorage.getItem("anxietyMoodLog");
    const savedJournal = localStorage.getItem("anxietyJournalLog");
  ...
  useEffect(() => {
    localStorage.setItem("anxietyHabitLog", JSON.stringify(completedHabits));
  }, [completedHabits]);

  useEffect(() => {
    localStorage.setItem("anxietyCustomHabits", JSON.stringify(customHabits));
  }, [customHabits]);

  useEffect(() => {
    localStorage.setItem("anxietyMoodLog", JSON.stringify(moodLog));
  }, [moodLog]);

  useEffect(() => {
    localStorage.setItem("anxietyJournalLog", JSON.stringify(journalLog));
  }, [journalLog]);
  ...
  return (
    <div className="min-h-screen bg-[#f5f5dc] p-6 flex flex-col items-center">
      <h1 className="text-2xl font-bold text-purple-600 mb-2">Anxiety Support</h1>
